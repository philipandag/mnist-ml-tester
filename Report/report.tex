...
\begin{center}
    \begin{tikzpicture}[every node/.style={minimum height=0.8cm, minimum width=1cm}]
        \matrix[draw=none, column sep=0.5cm, row sep=0.6cm]{
            \node[rectangle, draw] (input) {$28 \times 28$}; \\
            \node[rectangle, draw] (hidden1) {$32 \times (3, 3)$}; \\
            \node[rectangle, draw] (hidden2) {$64 \times (3, 3)$}; \\
            \node[rectangle, draw] (hidden3) {maxPooling(2,2)}; \\
            \node[rectangle, draw] (hidden4) {Flatten}; \\
            \node[rectangle, draw] (hidden5) {Dropout(0.5)}; \\
            \node[rectangle, draw] (output) {10}; \\
        };
        ...
        % Opisy etapów
        \node[below=0.3cm, right=0.0cm] at (input.south) {\tiny \textbf{relu}};
        \node[below=0.3cm, right=0.0cm] at (hidden1.south) {\tiny \textbf{maxPooling(2,2)}};
        \node[below=0.3cm, right=0.0cm] at (hidden2.south) {\tiny \textbf{relu}};
        \node[below=0.3cm, right=0.0cm] at (hidden5.south) {\tiny \textbf{softmax}};
        
        % Strzałki między węzłami
        \draw[->] (input) -- (hidden1);
        \draw[->] (hidden1) -- (hidden2);
        \draw[->] (hidden2) -- (hidden3);
        \draw[->] (hidden3) -- (hidden4);
        \draw[->] (hidden4) -- (hidden5);
        \draw[->] (hidden5) -- (output);
    \end{tikzpicture}
\end{center}
